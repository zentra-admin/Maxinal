...
export const localRuntime: RuntimeManager = {
  async start(projectLocation, requestedPort) {
    ...
    const child = spawn("npm", ["run", "start", "--", "-p", String(requestedPort ?? 0)], { cwd: projectLocation, stdio: ["ignore", "pipe", "pipe"], detached: true });
    let output = "";
    ...
    while (Date.now() < deadline) {
      ...
      if (actualPort) { try { const response = await fetch(`http://127.0.0.1:${actualPort}`); if (response.ok) { const url = `http://127.0.0.1:${actualPort}`; const session: RuntimeSession = { url, async stop() { return new Promise(resolve => { if (child.exitCode !== null) return resolve({ ok: true }); child.once("exit", () => resolve({ ok: true })); try { process.kill(-child.pid!, "SIGTERM"); } catch { child.kill("SIGTERM"); } }); } }; return { ok: true, session }; } } catch { /* still starting */ } }
